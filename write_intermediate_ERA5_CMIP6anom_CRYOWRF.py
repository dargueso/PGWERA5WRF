...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--resolution",
        type=str,
        help="Horizontal resolution in degrees of ERA5 files.",
        default="0.25",
    )
    ...
    parser.add_argument(
        ...
        "--reverse",
        type=str,
        help="Reverse the anomaly to sum instead to rest",
        default="False",
    )
...
vars2d = ["uas", "vas", "tas", "hurs", "ps", "psl", "tos", "siconc", "ts"]
vars2d_invarERA5 = ["rsn", "sd", "swvl1", "swvl2", "swvl3", "swvl4", "stl1", "stl2", "stl3", "stl4"]
...
for i,datetime in enumerate(datetimes):
    ...
    nt = 0# % int(24/int(datetime_step)) #step number into a daily file with many dates

    print("processing year %s month %02d day %02d hour %02d" % (year, month, day, hour))

    ferapl = nc.Dataset(
        "%s/pamir_pl_%s.nc" % (ERA5_dir, datetime.item().strftime(datetime_fileformat)), "r"
    )
    ferasfc = nc.Dataset(
        "%s/pamir_surface_%s.nc" % (ERA5_dir, datetime.item().strftime(datetime_fileformat)), "r"
    )
    ...
    if filewrite == True:
        ...
        for var in vars2d:
            ...
            if var == "hurs":
                ...
                var_era = ferasfc.variables["%s" % (vars2d_codes[var])][nt, :, :]*100

            else:
                var_era = ferasfc.variables["%s" % (vars2d_codes[var])][nt, :, :]

            #Change soil temperature by the same temperature than the surface to avoid physical problems
            if var in ["stl1", "stl2", "stl3", "stl4"]:
                fanom = nc.Dataset(
                    "%s/%s_CC_signal_%s_%s.nc"
                    % (CMIP6anom_dir, "ts", scenario_diff, date_diff)
                )
                ...
                if np.min(np.abs(tdelta)) == 0:
                    var_anom = fanom.variables["%s" % ("ts")][i1, :, :]
                else:
                    var_anom_1 = fanom.variables["%s" % ("ts")][i1, :, :]
                    var_anom_2 = fanom.variables["%s" % ("ts")][i2, :, :]
            ...
            if var == "siconc":
               # np.where(vout[var]>1) = 1
               # np.where(vout[var]<0) = 0
               # print(np.max(var_era))
               # print(np.max(var_anom))
               # print(np.max(vout))

                vout["siconc"][vout["siconc"] > 100] = 100
                vout["siconc"][vout["siconc"] < 0 ] = 0
        ...
        for var in vars2d_invarERA5:
            print("Processing variable %s" % (var))
            if var == "z":
                var_era = ferasfc.variables["%s" % (vars2d_codes[var])][nt, :, :]/9.81
                vout[var] = var_era
            else:
                var_era = ferasfc.variables["%s" % (vars2d_codes[var])][nt, :, :]
                vout[var] = var_era
        
        print(np.min(vout["tos"]))
        print(np.max(vout["siconc"]))
        ...
        startlat = lat[0]
        startlon = lon[0]
        deltalon = float(args.resolution)
        deltalat = -float(args.resolution)
        ...
        fields2d[8, :, :] = np.float32(vout["siconc"]/100) #unit correction for siconc
        
        #|fields2d[9, :, :] = np.float32(vout["z"])
        fields2d[9, :, :] = np.float32(vout["rsn"]*33.3333) #correction for snow density
        fields2d[10, :, :] = np.float32(vout["sd"]*3.3333) #correction for snow height
